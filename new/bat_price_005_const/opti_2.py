...
def first( load_profile_file=None, production_profile_file=None, capex_battery=0.03, injection_price=0.04):
  ...
  # Affiche les 3 valeurs de la première ligne sous forme de floats
    ...
    m.pv_area = Var(within=NonNegativeReals, bounds=(10, 3000), initialize=50)
    m.wh_battery = Var(within=NonNegativeReals, bounds=(0,500000), initialize=10000)
    ...
    m.kWc = Var(within=NonNegativeReals, bounds=(1.82, 546), initialize=71)
...
def main_simu():
    ...
    for n in range(1,49):  # adapte la liste à tes cas
        ...
        try:
            res = first(load_profile_file, production_profile_file, capex_battery=0.05)
            results.append(res)
    ...
    print(df)
    df.to_csv("C:/Users/simva/OneDrive/Documents/1 Master 2/Mémoire/code/memoire_partage_elec/new/resultats_simulations_bat_capex_005.csv", index=False)
