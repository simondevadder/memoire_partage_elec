...
class EnergyCommunity:
    ...
    def func_ev_charger(self):
        ...
        for timestep in range(35040):
            if self.is_on:
                if timestep >= self.last_timestep + self.time_on:
                    self.is_on = False
                    self.time_off = np.random.randint(1,17) #8.5 timestep mean for 2/3 of the time on 
                    self.last_timestep = timestep
            else: 
                if timestep >= self.last_timestep + self.time_off:
                    quart_day = timestep % 96
                    if quart_day <= 16 or quart_day >= 88 :  # night
                        self.time_on = np.random.randint(16,41)   # between 4h and 10h during the night
                    else :
                        self.time_on = np.random.randint(2,25)  # between 30min and 6h (26km/day -> 30min charge)
                    self.is_on = True
                    self.last_timestep = timestep
                    self.ev_powerarray[timestep : timestep + self.time_on] = self.ev_power
    ...
    def func_repartition(self, consumption, production):
        ...
        if self.key=="fix1round":
            ...
            for iteration in range(10):
                if still_to_repart<=0:
                    break
                if sum(conso_not_full) <= 0:
                    break
                remain = 0
